...
date: 2025-11-23
description: "本文详细介绍了使用Python爬取加密m3u8格式视频《奇迹少女伦敦篇》的全过程。通过分析网页结构获取m3u8文件地址，利用异步协程高效下载上千个AES加密的ts片段，使用获取的解密密钥进行批量解密，最后通过ffmpeg按顺序合并成完整mp4文件。技术要点包括：动态页面处理、正则匹配、异步下载、AES-CBC解密和视频合并。整个过程涉及requests、aiohttp、asyncio、Crypto和ffmpeg等工具，展示了处理加密视频资源的完整解决方案"
tags: "[python,爬虫]"
---

本次爬虫目标：爬取《某某少女某某篇》，并选择ukyun源进行，最后保存起来；
本次使用工具/方法：request、正则、协程、ffmpeg视频合并、AES解密
<!-- more -->
![img.png](../assets/img/25-11-23-img/img.png)
直接f12分析，发现他的资源是在iframe的，并且我们要爬取的是一个m3u8的文件
![img_1.png](../assets/img/25-11-23-img/img_1.png)
...
![img_2.png](../assets/img/25-11-23-img/img_2.png)
这个标签里边的链接就是全屏播放，如下：
![img_3.png](../assets/img/25-11-23-img/img_3.png)
对这个该页进行发送请求返回结果：
![img_4.png](../assets/img/25-11-23-img/img_4.png)
![img_5.png](../assets/img/25-11-23-img/img_5.png)
在返回的结果中，这个url对应的就是相应的m3u8文件，我们在浏览器播放视频时抓包也能看到，这个地址是包含相应的电影资源的
![img_6.png](../assets/img/25-11-23-img/img_6.png)
我们使用正则把该网址提取出来，并且保存一下
![img_7.png](../assets/img/25-11-23-img/img_7.png)
得到以下结果：
![img_8.png](../assets/img/25-11-23-img/img_8.png)
这也对应着网页监控中的相应内容
![img_9.png](../assets/img/25-11-23-img/img_9.png)
...
![img_10.png](../assets/img/25-11-23-img/img_10.png)
...
```
得到以下结果：
![img_11.png](../assets/img/25-11-23-img/img_11.png)
...
![img_12.png](../assets/img/25-11-23-img/img_12.png)
![img_13.png](../assets/img/25-11-23-img/img_13.png)
然后就是需要把这些视频合并在一起。因为是使用异步爬取的视频，所有顺序也是乱糟糟的，我们依旧读取m3u8文件，根据文件里的ts顺序来整理一下，写到一个txt文件里，然后再用ffmpeg根据txt文件里的顺序进行一次性合并
![img_14.png](../assets/img/25-11-23-img/img_14.png)
![img_15.png](../assets/img/25-11-23-img/img_15.png)
最后也是成功的将这上千个ts文件合并成一个mp4文件了
![img_16.png](../assets/img/25-11-23-img/img_16.png)
